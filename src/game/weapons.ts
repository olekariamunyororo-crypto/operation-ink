...
export class FirstPersonWeapons {
  ...
  private pose(_dt: number) {
    ...
    const bolt = this.model.userData.parts.bolt
    if (bolt && this.current.name === 'shotgun' && this.cooldown > 0) {
      const cycle = WEAPON_RULES.shotgun.interval - this.cooldown
      const pump = smooth(cycle, 0.08, 0.28) * (1 - smooth(cycle, 0.42, 0.62))
      bolt.position.z -= pump * 0.08
    }
    this.rightHand.position.set(0.02, -0.04, 0.02)
    this.leftHand.position.set(-0.08 - working * 0.04, -0.12 - working * 0.05, -0.15 - working * 0.08)
    this.placeArm(this.arms[0], this.rightHand.getWorldPosition(new THREE.Vector3()).sub(this.root.getWorldPosition(new THREE.Vector3())))
    this.placeArm(this.arms[1], this.leftHand.getWorldPosition(new THREE.Vector3()).sub(this.root.getWorldPosition(new THREE.Vector3())))
    if (this.reloading && magazine) {
      ...
      this.leftHand.position.copy(contact)
      this.placeArm(this.arms[1], contact)
    }
  }

  private shoot(item: WeaponItem) {
    if (item.magazine <= 0) {
      this.context.emit({ kind: 'empty', position: this.feet.clone(), radius: 2, text: 'Empty magazine' })
      this.held = false
      this.pendingShot = false
      return
    }
    ...
    const direction = camera.getWorldDirection(new THREE.Vector3())
    const pellets = item.name === 'shotgun' ? SHOTGUN_PELLETS : 1
    for (let i = 0; i < pellets; i++) {
      const spread = item.name === 'shotgun' ? SHOTGUN_BALLISTICS : { pitch: 0, yaw: 0 }
      const dir = direction.clone()
      if (item.name === 'shotgun') {
        dir.applyAxisAngle(right, (Math.random() - 0.5) * spread.pitch)
        dir.applyAxisAngle(up, (Math.random() - 0.5) * spread.yaw)
        dir.normalize()
      }
      this.context.onShot({
        origin: origin.clone(), direction: dir, range: rules.range, weapon: item.name,
        damage: rules.damage, pelletIndex: i,
      })
    }
    this.context.emit({ kind: `shot-${item.name}`, position: origin.clone(), radius: item.name === 'pistol' ? 38 : 55, text: `${rules.label} fired` })
    this.nudge(rules.recoilPitch ?? 0.02, (Math.random() - 0.5) * (rules.recoilYaw ?? 0.01))
  }

  private nudge(pitch: number, yaw: number) {
    this.settle.pitch += pitch
    this.settle.yaw += yaw
  }

  addPickup(item: WeaponItem) {
    if (this.disposed || this.loose.has(item.id) || this.inventory.some(i => i?.id === item.id)) return
    const model = createMissionGun(item.name)
    model.position.set(...(item.position ?? [0, 0, 0]))
    this.context.scene.add(model)
    this.loose.set(item.id, { item: copyItem(item), model })
  }

  pickupTargets() {
    const targets = []
    for (const [id, loose] of this.loose) {
      const item = loose.item
      targets.push({
        object: loose.model, point: loose.model.position.clone(),
        label: `${this.inventory.every(Boolean) ? 'Swap' : 'Take'} ${WEAPON_RULES[item.name].label}`, id,
      })
    }
    return targets
  }
  ...
  pickup(id: string) {
    const loose = this.loose.get(id)
    if (!loose) return false
    const item = copyItem(loose.item)
    ...
    disposeGun(loose.model)
    this.context.scene.remove(loose.model)
    this.setHeldModel()
    this.context.emit({ kind: 'pickup', position: this.feet.clone(), radius: 2, text: this.label })
    return true
  }
  ...
  drop(position: THREE.Vector3) {
    ...
    const dropped = copyItem(item)
    dropped.position = [position.x, position.y, position.z]
    this.addPickup(dropped)
    this.setHeldModel()
  }

  snapshot(): WeaponSnapshot {
    return { slots: this.inventory.map(item => item ? copyItem(item) : null), selected: this.slot,
      loose: [...this.loose.values()].map(l => copyItem({ ...l.item, position: [l.model.position.x, l.model.position.y, l.model.position.z] })) }
  }

  restore(snapshot: WeaponSnapshot) {
    this.cancel()
    for (const loose of this.loose.values()) { disposeGun(loose.model); this.context.scene.remove(loose.model) }
    this.loose.clear()
    ...
    for (const item of snapshot.loose ?? []) this.addPickup(item)
    this.setHeldModel()
  }
  ...
  dispose() {
    ...
    if (this.model) disposeGun(this.model)
    this.model = null
    this.context.camera.remove(this.root)
  }
}
